...
def exp_same_n_zealots(plot, seeds):
    for p in ZEALOT_PERCENTAGES:
        ...
        print(f"Zealot % {p:.2f} - mean: {np.mean(probs):.3f}, std: {np.std(probs):.3f}")
...
def exp_vary_x_zealots(plot, seeds):
    for p in ZEALOT_PERCENTAGES:
        ...
        for seed in seeds:
            ...
            probs.append(result['robust_consensus_prob'])
        print(f"X Zealots % {p:.2f} - mean: {np.mean(probs):.3f}, std: {np.std(probs):.3f}")
...
def exp_vary_y_zealots(plot, seeds):
    for p in ZEALOT_PERCENTAGES:
        ...
        for seed in seeds:
            ...
            probs.append(result['robust_consensus_prob'])
        print(f"Y Zealots % {p:.2f} - mean: {np.mean(probs):.3f}, std: {np.std(probs):.3f}")
...
def exp_vary_group_size(plot, seeds):
    for g in GROUP_SIZES:
        ...
        for seed in seeds:
            ...
            probs.append(result['robust_consensus_prob'])
        print(f"Group size {g} - mean: {np.mean(probs):.3f}, std: {np.std(probs):.3f}")
...
def exp_small_group_sizes(plot, seeds):
    for g in SMALL_GROUP_SIZES:
        ...
        for seed in seeds:
            ...
            probs.append(result['robust_consensus_prob'])
        print(f"Small group size {g} (10% zealots) - mean: {np.mean(probs):.3f}, std: {np.std(probs):.3f}")
